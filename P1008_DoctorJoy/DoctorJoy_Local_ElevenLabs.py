...
# --- Configuración del reconocimiento de voz (ElevenLabs Scribe) -----
# Modelo de transcripción de ElevenLabs. "scribe_v1" es el estable;
# "scribe_v2" es la versión más reciente si está disponible en tu cuenta.
ELEVENLABS_STT_MODEL_ID = os.getenv("ELEVENLABS_STT_MODEL_ID", "scribe_v1")
# Código de idioma (ISO-639-1) del paciente. Cámbialo en el .env con
# ELEVENLABS_STT_LANGUAGE_CODE si el paciente habla otro idioma.
ELEVENLABS_STT_LANGUAGE_CODE = os.getenv("ELEVENLABS_STT_LANGUAGE_CODE", "es")
...
def obtener_cliente_elevenlabs():
    ...
    Este mismo cliente se usa tanto para texto a voz (text_to_speech)
    como para voz a texto (speech_to_text).
    """
    global _cliente_voz
...
def escuchar(reconocedor, microfono):
    """Escucha por el micrófono y devuelve el texto reconocido (vía ElevenLabs Scribe), o None si falla."""
    cliente_voz = obtener_cliente_elevenlabs()
